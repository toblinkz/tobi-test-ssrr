												...
												<div class="row ">
													<center>
														...
														<div class="panel-transparent">
															<p id="welcome" style="margin-top: 10px;margin-bottom: 15px"><i class="entypo-list-add"></i> Register
																your device ID!</p>
															<p class="insight hidden-xs">Register your device to push out WhatsApp messages. <br>Approved device ID's
																are required to send messages on Termii!</p>
															...
															<div class="row">
																<center>
																	...
																	<div class="col-sm-4">
																		<br>
																		<a v-if="canRequestDevice" class="btn btn-primary" @click="showModal"><i class="fa fa-plus"></i> Make
																			a new request</a>
																	</div>
																	<div class="col-sm-4 hidden-xs">
																	</div>
														...
														<table class="table table-responsive  table-hover">
															...
															<tr v-for="(row, index) in response_data.data" :key="row.id">
																...
																<td data-label="Status">
																	<button :id="row.id" @click="getQRCode(row.id)"
																									style="background: #4caf50; border: 1px solid #4caf50; border-radius: 3px;"
																									:class="barcodeDisabled(row)" v-show="showBarcodeIcon(row)"><i class="fa fa-barcode"
																																																																																								style="color: #fff;"></i>
																	</button>
																	<p v-show="lockBarcode(row)"><i class="entypo-lock" style="color: red;"></i></p>
																</td>
																<td v-if="canViewDevice" data-label="view subscriptions">
																	<nuxt-link class="btn btn-primary" :aria-disabled="isDisabled(row)"
																												:to="{name: 'device-id-subscriptions', params:{name: row.name, id: row.id}}">manage device
																	</nuxt-link>
																</td>
															</tr>
															</tbody>
...
export default {
	...
	methods: {
		...
		async fetch() {
			try {
				this.response_data = await this.$axios.$get('devices', {params: {page: this.page}});
				this.page = this.response_data.meta.current_page;
				this.showPagination = this.response_data.meta.last_page <= 1;
